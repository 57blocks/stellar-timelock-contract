...
async function updateGrantRoleDirectly(roleAccount,role) {
  let timeLockOwnerLabel = users.timelockOwner;
  let timeLockOwnerSecret = process.env[`${timeLockOwnerLabel}_secret`];
  ...
  console.log(`before execute grant role: ${hasRole}`);
  await invokeContract(keyPair, timeLockContractId, "grant_role", params);
  hasRole = await invokeContract(keyPair, timeLockContractId, "has_role", params);
  console.log(`after execute grant role: ${hasRole}`);
}

async function main(roleAccount,role) {
  await scheduleAndExecuteGrantRole(roleAccount,role);
  await updateGrantRoleDirectly(roleAccount,role);
}

module.exports = {
  addRoleToTimeLock: main,
};
