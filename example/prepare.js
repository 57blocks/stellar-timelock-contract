...
(async () => {
  const timelockPath = '../target/wasm32-unknown-unknown/release/time_lock.wasm';
  if (!fs.existsSync(timelockPath)) {
    await cmd(`soroban contract build`);
    await cmd(`soroban contract optimize --wasm ../target/wasm32-unknown-unknown/release/time_lock.wasm`);
  }
  await cmd(
  ...
  let tokenContractId = await deployContract(
    `${getRootPath()}/example/wasm/soroban_token_contract.optimized.wasm`,
    'mock_token'
  );
  let timelockContractId = await deployContract(
    `${getRootPath()}/target/wasm32-unknown-unknown/release/time_lock.optimized.wasm`,
    'timelock'
  );
  let timelockContractId2 = await deployContract(
    `${getRootPath()}/target/wasm32-unknown-unknown/release/time_lock.optimized.wasm`,
    'timelock_self_managed'
  );
  ...
  await cmd(
    `soroban contract invoke --id ${timelockContractId} --network ${
      networks.name
    } --source ${deployer} -- initialize --min_delay ${minDelay} --proposers '{"vec":[{"address":"${proposerPubKey}"}]}' --executors '{"vec":[{"address":"${executorPubKey}"}]}' --owner ${null}`
  );
  ...
  async function createAccount(name) {
    ...
    try {
      ...
      throw new Error(`Issue with ${keypair.publicKey()} account. Ensure you're running the \`./docker.sh\` network.`);
    }
